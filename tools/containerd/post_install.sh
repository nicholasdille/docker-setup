...
if ! test -f "/etc/containerd/config.toml"; then
    ...
    sed -i "s|/opt/cni/bin|${relative_target}/libexec/cni|" "/etc/containerd/config.toml"
...
sed -i "s|CONTAINERD=/usr/local/bin/containerd|CONTAINERD=${relative_target}/bin/containerd|" "/etc/init.d/containerd"
echo "Patch systemd unit"
sed -i "s|ExecStart=/usr/local/bin/containerd|ExecStart=${relative_target}/bin/containerd|" "/etc/systemd/system/containerd.service"
